...
impl ImgProcessor {
    pub fn setup_canny_edge_detection(&self, low_threshold: f32, high_threshold: f32) -> Canny {
        Canny::new(low_threshold, high_threshold, self.path.clone(), &self.context, self.queue.clone(), self.dim)
    }
}